...
- **同步拓扑**：优先**局域网直连**（mDNS 发现 + QUIC 通道），无公网服务器依赖；后续可选自托管中继（类似 Syncthing relay）解决跨网段场景。
- **配对与加密**（不可省略）：首次配对采用扫码/PIN 交换验证，设备各持密钥对，通道用 TLS1.3/QUIC 自签证书互相钉扎（KDE Connect 模式），同步条目端到端加密落盘可选。剪贴板内容包含密码与验证码，明文传输/存储不可接受。
...
2. **Android 剪贴板限制**：Android 10+ 后台应用不可读剪贴板，同步的触发点只能是"输入法激活时"，产品预期要按此设计。
