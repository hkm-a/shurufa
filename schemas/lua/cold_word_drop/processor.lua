...
function processor.init(env)
    ...
    -- 与 filter.lua 保持一致：降频词表以 processor 实际写入的 reduce_freq_words.lua 为准。
    -- turn_down_words.lua 为空文件时 require 返回 true，旧 or 链会取到布尔值导致后续
    -- reduce_freq_words[word] 索引崩溃、候选流清空（2026-08-18 实测）。
