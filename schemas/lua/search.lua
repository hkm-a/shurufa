-- 部件拆字辅码筛选（rime-ice search.lua 同款，Mirtle 授权协议）
...
-- 并在方案里配置（见 rime_ice.schema.yaml）：
--   key_binder:
--     search: "`"           # 辅码引导符（search.lua 自行读取，非 librime 内置）
--   speller:
--     alphabet: ...`        # 引导符需加入 alphabet 才能进入输入码
--     initials: ...         # 引导符**不**加入 initials，避免单独成码
--
-- 功能：输入拼音出现候选后，输入引导符 + 部件码，按 radical_pinyin 词典
-- 过滤候选（如 nihao`ren → 只留首字含 亻(ren) 部件的候选 你好/倪/伲…），
-- 类似搜狗/微软的候选内笔画、部件筛选。支持：
-- - 方案反查（schema 查字，支持算法转换过的编码）：search/schema
-- - 数据库反查（db 查 reverse.bin，快，仅硬编码）：search/db
-- - 双辅码（db 反查时第二个引导符匹配词组第二字或单字正则）
-- - show_other_cands：不匹配的候选置后而非隐藏
