...
当前版本 **v1.4.0（versionCode 36，2026-08-19）**：PC 侧 M1–M11 路线图全部完成
（v1.2.0 读屏无障碍阶段二追加：候选窗 ITextProvider + UIA 运行时探针）；安卓侧
按搜狗安卓时间线推进：M-A1「键盘形态与输入效率」（v1.3.0 验收：键盘快捷设置/
高度/按键音振动/单手模式/9 键 T9 拼音）→ M-A2「表达与效率工具」（v1.4.0 验收：
常用语/表情面板/时间日期邮箱/计算器/剪贴板增强），路线图见
